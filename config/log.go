...
func init() {
	config.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			// 日志级别，必须是以下选项：
			// "debug" —— 信息量打，一般调试时打开。系统模块下详细的运行的日志，例如 HTTP 请求、数据库请求、发送邮件、发送短信
			// "info" —— 业务级别的运行日志，如用户登录、用户退出、订单撤销。
			// "warn" —— 感兴趣、需要引起关注的信息。例如，调试时候打印调试信息（命令行输出会有高亮）。
			// "error" —— 记录错误信息。Panic 或者 Error。如数据库连接错误、HTTP端口被占用等。一般生产环境使用的等级。
			...
			// 最多保存日志文件数，0 为不限，MaxAge 到了也会删除
			"max_backup": config.Env("LOG_MAX_BACKUP", 5),
			// 最多保存七天，7 表示一周前的日志会被删除，0 表示不删
