...
func InitLogger(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string, level string) {
	...
	Logger = zap.New(
		...
		zap.AddCallerSkip(1),              // 封装了一层，调用文件去除一层(runtime.Caller)
		zap.AddStacktrace(zap.ErrorLevel), // Error 时才会显示 stacktrace
	)

	// 将自定义的 logger 设置为全局的 logger
...
func getEncoder() zapcore.Encoder {
	...
	if app.IsLocal() {
		...
		// 本地设置内置的 Console 解码器（支持stacktrace 换行）
...
func customTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format("2006-01-02 12:04:05"))
}
...
func getLogWriter(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string) zapcore.WriteSyncer {
	...
	// 如果配置了按照日期记录日志文件
	if logType == "daily" {
		logname := time.Now().Format("2006-01-01.log")
...
// Fatal 级别同 Errot(),写完 log 后调用 os.Exit(1) 退出程序
...
// DebugJSON 记录对象类型的 debug 日志，使用json.Marshal 进行编码。
// 调用示例：
//			logger.DebugString("Auth","读取登录用户",auth.CurrentUser())
